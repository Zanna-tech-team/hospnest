...
function TeamPage() {
  ...
  const inviteMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || "Failed to send staff invitation"),
  });
  ...
  const updateShiftsMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["team-context"] });
      queryClient.invalidateQueries({ queryKey: ["staff-detail"] });
      toast.success("Weekly shift schedule updated and synced.");
    },
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Manage clinical staff, weekly shift rosters, and active on-duty permissions.
          </p>
        </div>
        ...
        {/* Successful Invite Banner Modal */}
        {lastInviteResult && (
          <div className="mt-6 rounded-2xl border border-teal/30 bg-teal/5 p-5 shadow-soft">
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              <div className="flex items-start gap-3">
                <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-teal text-white">
                  <UserCheck className="size-5" />
                </span>
                <div>
                  <h3 className="font-display font-semibold text-foreground">
                    Invite link ready for {lastInviteResult.fullName} ({lastInviteResult.role})
                  </h3>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Send this onboarding link to the team member so they can activate their account.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
