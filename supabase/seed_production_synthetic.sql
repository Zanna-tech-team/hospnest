...
BEGIN
  ...
  -- 5.1 SEED DEDICATED TEST PATIENT (patient@example.com / Password123!)
  DECLARE
    test_pat_auth_id UUID;
    test_pat_id UUID := '77777777-7777-4777-8777-777777777777';
  BEGIN
    SELECT id INTO test_pat_auth_id FROM auth.users WHERE email = 'patient@example.com' LIMIT 1;
    IF test_pat_auth_id IS NULL THEN
      test_pat_auth_id := gen_random_uuid();
      INSERT INTO auth.users (
        ...
        now(),
        '{"provider":"email","providers":["email"]}'::jsonb,
        jsonb_build_object('full_name', 'Chinedu Patient Obi', 'role', 'patient'),
        now(), now(), '', '', '', ''
      );
      ...
      INSERT INTO auth.identities (
        ...
        test_pat_auth_id, test_pat_auth_id,
        jsonb_build_object('sub', test_pat_auth_id::text, 'email', 'patient@example.com'),
        'email', test_pat_auth_id::text, now(), now(), now()
      ) ON CONFLICT (provider, provider_id) DO NOTHING;
    ELSE
      UPDATE auth.users SET
        encrypted_password = extensions.crypt('Password123!', extensions.gen_salt('bf')),
        email_confirmed_at = COALESCE(email_confirmed_at, now())
      WHERE id = test_pat_auth_id;
    END IF;

    -- Upsert patient record for patient@example.com
    INSERT INTO public.patients (
      id, user_id, nin, first_name, last_name, date_of_birth, gender, phone, email,
      blood_group, genotype, allergies, chronic_conditions, emergency_contact, created_at
    ) VALUES (
      test_pat_id, test_pat_auth_id, '10000000001', 'Chinedu', 'Obi', '1992-06-15', 'Male',
      '+2348011223344', 'patient@example.com', 'O+', 'AA',
      ARRAY['Penicillin', 'Sulphonamides'], ARRAY['Mild Asthma'],
      '{"next_of_kin_name": "Ngozi Obi", "relationship": "Spouse", "phone": "+2348011223399"}'::jsonb,
    ...
    -- Map patient role
    INSERT INTO public.user_roles (
      user_id, hospital_id, role, is_active
    ) VALUES (
      test_pat_auth_id, NULL, 'patient'::public.user_role_type, TRUE
    ) ON CONFLICT (user_id, hospital_id, role) DO UPDATE SET is_active = TRUE;

    -- Consent at both hospitals
    INSERT INTO public.patient_consents (patient_id, hospital_id, scope, is_active, expires_at)
    VALUES 
      (test_pat_id, hosp_a, 'full_record', TRUE, now() + interval '365 days'),
      (test_pat_id, hosp_b, 'full_record', TRUE, now() + interval '365 days')
    ON CONFLICT DO NOTHING;
  END;
